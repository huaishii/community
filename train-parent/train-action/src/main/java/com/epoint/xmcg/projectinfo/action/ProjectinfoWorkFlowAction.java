 ...
 */
@RestController("projectinfoworkflowaction")
@Scope("request")
public class ProjectinfoWorkFlowAction extends WorkflowBaseController
{
    private static Logger logger = LogManager.getLogger(ProjectinfoWorkFlowAction.class);
    ...
    // 数据表名
    private String SQLTableName = "ProjectInfo";
    ...
    public List<SelectItem> getBelongzoneModel() {
        if (belongzoneModel == null) {
            belongzoneModel = DataUtil.convertMap2ComboBox(
                    (List<Map<String, String>>) CodeModalFactory.factory("下拉列表", "所属地区", null, true));
        }
